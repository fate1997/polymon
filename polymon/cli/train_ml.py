...
def parse_args():
    ...
    parser.add_argument('--raw-csv', type=str, default='database/internal/train.csv')
...
def train(
    ...
    logger.info(
        f'Train inf: {len(x_train) - mask_train.sum()}, '
        f'Val inf: {len(x_val) - mask_val.sum()}, '
        f'Test inf: {len(x_test) - mask_test.sum()}'
    )
...
def main(args: argparse.Namespace):
    ...
    for label in args.labels:
        scaling_error, n_test = train(
            ...
            optimize_hparams=args.n_trials is not None,
