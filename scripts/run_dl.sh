...
python "$SCRIPT_DIR/main_dl.py" --num-epochs 1000 --model $MODEL --raw-csv "$SCRIPT_DIR/../database/external/Rg_relative_difference_merged.csv" --labels Rg --run-production --optimize-hparams --tag $TAG --early-stopping-patience 250 --n-trials $N_TRIALS --n-fold $N_FOLD $ADDI_ARGS
